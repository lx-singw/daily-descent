...
12. **"Last survivor" silhouette**: Render a faint static marker showing where the current daily leaderboard leader reached their deepest point.
...
14. **Free-Form Spirit Messages**: Allow players to leave custom text messages on death. Implements report/removal tools and user verification.
15. **Comment-Derived Draft Cards**: Scan top-voted comments using a strict whitelist parser (numeric bounds only) and compile them into tomorrow's draft pool.
16. **Flair & Badge Milestones**: Award subreddit flair milestones for first clears, top finishes, or streak counts.
17. **Weekly leagues**: Track weekly halls of fame across date keys.
...
18. **Personalization Layer**: Quiet tendency modeling (analyzing player habits) to emphasize specific ghost trails or cards, without altering the shared daily seed.
19. **Automated Curation Pipeline**: Auto-balance checking of card effects against historical completion statistics.
20. **Cross-Game Portability**: Portable player profiles and achievements across multiple Devvit games.
